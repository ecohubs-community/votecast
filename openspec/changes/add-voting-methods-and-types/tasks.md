...
- [x] 4.6 All legacy columns dropped: `strategy_id`/`choice_id`/`metadata_json` (migration 0004) and `status` (migration 0006). Lifecycle, list filter (now time-based phase categories), draft-guards, and UI all run on `phase`. local.db migrated; 193 tests green.
