# governance-events Specification

## Purpose

The lifecycle event catalog for a proposal's process and the subscribers wired to it: in-app
notifications (respecting visibility policy), ordered webhook/execution-handler side-effect
pipelines, and the external-resolver boundary for delegating a decision to community-hosted code
